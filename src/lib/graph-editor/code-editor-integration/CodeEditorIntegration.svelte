<script lang="ts">
  ...
  async function push() {
    ...
    const rootTypes = [
      "Problem",
      ...Object.keys(geosSchema.tree["Problem"] ?? {}),
    ];
    const factory = editorContext.activeFactory;
    if (!factory) throw new ErrorWNotif("No active editor");
    ...
    async function xmlToEditor(
      ...
      for (const xmlNode of xml) {
        ...
        for (const [k, v] of Object.entries(
          xmlAttributes as Record<string, string>
        )) {
          ...
          if (
            attrDef.type.startsWith("real") &&
            attrDef.type.endsWith("array2d")
          ) {
        ...
        for (const [k, a] of arrayAttrs.entries()) {
          ...
          if (!attrDef)
            throw new ErrorWNotif(
              "Couldn't find simple type for array attribute"
            );
        ...
        if (parent) {
          ...
          const conn = await tmp_editor.addNewConnection(
            node,
            "value",
            parent,
            "children"
        ...
        await xmlToEditor(
          (xmlNode as Record<string, ParsedXmlNodes>)[xmlTag],
          schema,
          node
        );
      }
    ...
    for (const { source, target } of groupNameLinks) {
      ...
      if (!getNameNodes.has(source)) {
        ...
        await tmp_editor.addNewConnection(
          sourceNode,
          "value",
          getNameNode,
          "xml"
    ...
    if (area) {
      wu(area.nodeViews.values()).forEach((nodeView) => {
        leftBound = Math.min(
          leftBound,
          nodeView.position.x - nodeView.element.offsetWidth * 1.4
        );
      });
    }
    ...
    for (const [i, nodeSaveData] of nodesSaveData.entries()) {
      ...
      if (nodeView)
        await nodeView.translate(
          tmp_nodeView.position.x + leftBound,
          tmp_nodeView.position.y
        );
    ...
    if (tmp_editor.getNodes().length > 0)
      notifications.show({
        ...
        message: get(_)(
          "code-editor-integration.push-to-graph.selected-node-dragging.message"
        ),
        autoClose: 7000,
      });
      ...
      .map(async (node) => {
        ...
        element.setAttribute(
          ...
          "data:text/plain;charset=utf-8," +
            encodeURIComponent(formatXml({ xml }))
        );
